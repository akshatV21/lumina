...
export class StorageService {
  ...
  async upload(buffer: Buffer, bucket: string, path: string, contentType: string = 'image/webp') {
    const res = await this.client.storage.from(bucket).upload(path, buffer, { contentType, upsert: true })
